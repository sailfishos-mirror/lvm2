...
/*
 * Execute a single DM ioctl against an already-built dmi buffer.
 * Sets dmt->ioctl_errno on failure.
 * Called from both the serial path (_do_dm_ioctl) and async worker threads.
 */
int dm_ioctl_exec(int fd, struct dm_task *dmt, struct dm_ioctl *dmi)
{
	...
	dmt->ioctl_errno = 0;
	r = ioctl(fd, dm_task_get_ioctl_cmd(dmt), dmi);
	if (r < 0)
 ...
 * Used by async backends (thread pool, sync) to avoid duplicating
 * the retry loop.
 */
int dm_ioctl_exec_retry(int fd, struct dm_task *dmt)
{
	...
	dm_ioctl_exec(fd, dmt, dmt->dmi.v4);

	if (dmt->ioctl_result < 0 && dmt->retry_remove &&
	    dmt->ioctl_errno == EBUSY) {
		for (retry = 0; retry < DM_IOCTL_RETRIES; retry++) {
			usleep(DM_RETRY_USLEEP_DELAY);
			dm_ioctl_exec(fd, dmt, dmt->dmi.v4);
			if (dmt->ioctl_result >= 0 ||
			    dmt->ioctl_errno != EBUSY)
				break;
		}
	}

	return dmt->ioctl_result;
}

/*
 * Shared post-ioctl processing for both serial and async paths.
 * Handles error logging, uevent completion, and name/uuid unmangling.
 * Sets *retryable (may be NULL in the async path) when EBUSY is seen.
...
#ifdef DM_IOCTLS
	r = dm_ioctl_exec(_control_fd, dmt, dmi);

	if (dmt->record_timestamp)
		if (!dm_timestamp_get(_dm_ioctl_timestamp))
			stack;

	if (_dm_ioctl_post(dmt, dmi, r, retryable))
		return dmi;
	stack;
#endif
	_dm_zfree_dmi(dmi);
...
/* DM_IOCTL_RETRIES and DM_RETRY_USLEEP_DELAY are in libdm-async.h */
...
{
	if ((unsigned) dmt->type >= DM_ARRAY_SIZE(_cmd_data_v4)) {
		log_debug(INTERNAL_ERROR "unknown device-mapper task %d",
			  dmt->type);
		return 0;
	}
...
int dm_task_prepare(struct dm_task *dmt)
{
	...
	if (!dm_task_get_ioctl_cmd(dmt))
		return_0;

	if (!(dmi = _dm_task_build_dmi(dmt, _ioctl_buffer_double_factor, 0)))
		return_0;
 ...
 * dmt->ioctl_result and dmt->ioctl_errno are set by dm_ioctl_exec()
 * or dm_ioctl_exec_retry() in the worker/sync backend.
 * Must be called from the main thread only -- node ops are not thread-safe.
 * Returns 1 on success, 0 on failure.
 *
 * DM_BUFFER_FULL_FLAG retry is handled here (main thread) rather than in
 * the backends, so the global _ioctl_buffer_double_factor is safe to touch.
 */
int dm_task_handle_completion(struct dm_task *dmt, void *userdata)
{
	...
	if (!_dm_ioctl_post(dmt, dmi, dmt->ioctl_result, NULL)) {
		_udev_complete(dmt);
		return_0;
	}

	while (dmi->flags & DM_BUFFER_FULL_FLAG) {
		switch (dmt->type) {
		case DM_DEVICE_LIST_VERSIONS:
		case DM_DEVICE_LIST:
		case DM_DEVICE_DEPS:
		...
		case DM_DEVICE_TARGET_MSG:
			...
			if (!dm_task_prepare(dmt))
				break;
			dm_ioctl_exec(_control_fd, dmt, dmt->dmi.v4);
			dmi = dmt->dmi.v4;
			if (!_dm_ioctl_post(dmt, dmi, dmt->ioctl_result, NULL)) {
				_udev_complete(dmt);
				return_0;
	...
	/* Chain next work via callback if set */
	if (dmt->async.complete_fn)
		return dmt->async.complete_fn(dmt, userdata);
		   ...
		   void *userdata)
{
	...
	while (ctx->fn_try_wait &&
	       ctx->fn_try_wait(ctx, &done, &done_ud)) {
 ...
 * On return, *dmt_out is the task, *userdata_out is the caller context
 * passed to dm_task_submit().
 * The ioctl result is stored in dmt->ioctl_result; call
 * dm_task_handle_completion() to do post-processing.
...
{
	return ctx->fn_wait(ctx, dmt_out, userdata_out);
}
