...
/* Accessed only from the main thread: dm_task_run() (serial path),
 * dm_async_submit() -> dm_task_prepare(), and dm_async_task_completion()
 * (which the API documents as main-thread-only).  No synchronisation needed. */
...
int dm_task_prepare(struct dm_task *dmt)
{
	...
	if (!(dmi = _dm_task_build_dmi(dmt, _ioctl_buffer_double_factor)))
		return_0;
		    ...
		    dm_async_complete_fn complete_fn, void *userdata)
{
	...
	while (ctx->fn_wait(ctx, DM_WAIT_POLL, &done, &done_data)) {
		if (!dm_async_task_completion(ctx, done, done_data)) {
			stack;
			ctx->submit_drain_error = 1;
		}
	}
 ...
 * in the worker thread (single ioctl, no retry).
 * Must be called from the main thread only -- node ops are not thread-safe.
 *
 * Takes ownership of dmt: destroys it on completion or re-submits it
 * back to ctx on DM_BUFFER_FULL_FLAG or EBUSY retry (task stays in flight).
 *
 * Returns 1 on success, 0 on failure.
 */
			      ...
			      void *userdata)
{
	...
	/* EBUSY retry for REMOVE: resubmit from main thread.
	 * No need to rebuild dmi -- kernel only overwrites the
	 * version field on EBUSY (4.0.0 -> running kernel version),
	 * which is idempotent on subsequent retries. */
	if (dmt->ioctl_result < 0 &&
	    dmt->ioctl_errno == EBUSY &&
	    dmt->type == DM_DEVICE_REMOVE &&
	    dmt->retry_remove &&
	    dmt->retry_remove <= DM_IOCTL_RETRIES) {
#ifdef DM_DEBUG_EBUSY_HOLDERS
		if (dmt->retry_remove == 1)
			_debug_show_holders(dmi);
#endif
		log_debug_activation("EBUSY retry %d/%u for %s (async).",
				     dmt->retry_remove, DM_IOCTL_RETRIES,
				     dmi->name);
		dmt->retry_remove++;
		if (ctx->fn_submit(ctx, dmt, userdata))
			return 1; /* re-submitted, dmt still in flight */
		/* submit failed -- fall through to post-processing */
	}

	if (!_dm_ioctl_post(dmt, dmi, dmt->ioctl_result)) {
		_udev_complete(dmt);
		ok = 0;
		goto_out;
	}
	...
	if (dmi->flags & DM_BUFFER_FULL_FLAG) {
		if (_can_retry_buffer_full(dmt) &&
		    dm_task_prepare(dmt) &&
		    ctx->fn_submit(ctx, dmt, userdata))
			return 1; /* re-submitted, dmt still in flight */
		ok = 0;
		goto_out;
	}

	if (!(ok = _dm_task_node_ops(dmt, dmi)))
		stack;

out:
	/* Always invoke callback so it can clean up userdata */
	if (dmt->async_complete_fn)
		ok = dmt->async_complete_fn(ctx, dmt, ok, userdata) && ok;
...
/*
 * Create an async context for async ioctl submission.
 * max_inflight is the number of ioctls that may be in-flight simultaneously.
 * Falls back to synchronous single-slot context when max_inflight == 1.
...
struct dm_async_ctx *dm_async_ctx_create(unsigned max_inflight)
{
	...
	return dm_async_ctx_alloc_threads(_control_fd, max_inflight);
}
