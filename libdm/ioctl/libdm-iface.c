...
#ifdef DM_IOCTLS
	r = dm_ioctl_exec(_control_fd, command, dmt, dmi);
 ...
 * ioctl_result is the return value of ioctl(); dmt->ioctl_errno is set
 * by dm_ioctl_exec() in the worker thread.
 ...
 *
 * NOTE: EBUSY retry (dm_task_run retries DM_DEVICE_REMOVE up to
 * DM_IOCTL_RETRIES times when dmt->retry_remove is set) is NOT handled
 * here.  The caller is responsible: after dm_async_wait_completion()
 * returns a failed task, check dmt->ioctl_errno == EBUSY and
 * dmt->retry_remove, then re-prepare and re-submit the task if needed.
 * Collect all completions first, then retry the EBUSY subset serially or
 * in a new async round -- do not mix retries with the in-flight batch.
 */
int dm_task_handle_completion(struct dm_task *dmt, int ioctl_result)
{
	...
	if (!_dm_ioctl_post(dmt, dmi, ioctl_result, NULL)) {
		_udev_complete(dmt);
		return_0;
	}
