...
int lm_init_vg_sanlock(char *ls_name, char *vg_name, uint32_t flags, char *vg_args, int opt_align_mb, char *other_args)
{
	...
	if (daemon_test) {
		...
		rv = dm_snprintf(vg_args, MAX_ARGS+1, "%s:%s", VG_LOCK_ARGS_V1, lock_lv_name);
		if (rv < 0)
			log_debug("init_vg_san vg_args may be too long %s", vg_args);
		return 0;
	}
	...
	rv = dm_snprintf(vg_args, MAX_ARGS+1, "%s:%s%s%s%s",
	...
	if (rv < 0) {
		log_error("S %s init_vg_san vg_args string too long %s", ls_name, vg_args);
		return -EINVAL;
	}
...
int lm_hosts_sanlock(struct lockspace *ls, int notify, int *hosts_unknown)
{
	...
	for (i = 0; i < hss_count; i++) {
		...
		if (state == SANLK_HOST_LIVE)
			found_live++;
		if (state == SANLK_HOST_UNKNOWN)
			found_unknown++;
		hs++;
	}
	free(hss);

	found_others = found_live + found_unknown;
	...
	if (hosts_unknown && found_unknown && !found_live)
		*hosts_unknown = 1;
...
int lm_setlockargs_vg_sanlock(char *ls_name, char *vg_name, struct action *act)
{
	...
	rv = sanlock_write_lockspace(&ss, 0, SANLK_WRITE_LS_FLAGS, sanlock_io_timeout);
	if (rv < 0) {
	...
	rv = dm_snprintf(act->vg_args, MAX_ARGS+1, "%s:%s%s%s%s",
		      (no_timeout || persist || no_watchdog) ? VG_LOCK_ARGS_V2 : VG_LOCK_ARGS_V1,
		      lock_lv_name,
		      no_timeout ? ":notimeout" : "",
		      no_watchdog ? ":nowatchdog" : "",
	...
	if (rv < 0) {
		log_error("S %s setlockargs vg_args string too long %s", ls_name, act->vg_args);
		return -EINVAL;
	}
