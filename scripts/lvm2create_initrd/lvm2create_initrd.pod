...
lvm2create_initrd creates an initial ramdisk (initrd) image suitable for booting to a system that has an LVM2 volume as its root filesystem.
...
Most of the parameters that can be set via command-line options can also be set
...
option is used then B<mdadm> program must be installed. This can also be
...
the maintainer if your distribution doesn't work with any of the current options.
