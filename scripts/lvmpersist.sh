...
do_start() {
	...
	for dev in "${DEVICES[@]}"; do
		if key_is_on_device "$dev" "$OURKEY" ; then
			...
			logmsg "Use lvmpersist stop, or check for multiple hosts using same key."
			exit 1
		fi
	done
...
usage() {
	...
	echo ""
	echo "${SCRIPTNAME} start --ourkey KEY --prtype PRTYPE --vg|--device ..."
	echo ""
	echo "${SCRIPTNAME} stop --ourkey KEY --vg|--device ..."
	echo ""
	echo "${SCRIPTNAME} preempt-abort --ourkey KEY --key KEY --vg|--device ..."
	echo ""
	echo "${SCRIPTNAME} devtest [--ourkey KEY] --vg|--device ..."
	echo ""
	echo "${SCRIPTNAME} clear --ourkey KEY --vg|--device ..."
	echo ""
	echo "Options:"
	echo "    --ourkey KEY"
