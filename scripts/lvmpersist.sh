...
get_key_list() {
	...
	if $cmdpersist $cmdopts --in --read-keys "$dev" 2>/dev/null | grep -q "there are NO registered reservation keys"; then
		KEYS=""
		return
	fi
...
get_current_reservation_holder() {
	...
	str=$( $cmdpersist $cmdopts --in --read-reservation "$dev" 2>/dev/null | grep -e "Key\s*=\s*0x" | xargs )
	...
	if [[ -z $str ]]; then
		if no_reservation_held $dev; then
			...
			logmsg "$cmdpersist read-reservation holder output not found $dev"
			HOLDER=0
		fi
		false
		return
...
get_current_reservation_desc() {
	...
	if [[ -z $str ]]; then
		if no_reservation_held $dev; then
			...
			logmsg "$cmdpersist read-reservation type output not found $dev"
			DESC=error
		fi
		false
		return
...
do_takeover() {
	...
	for dev in "${DEVICES[@]}"; do
		...
		if [[ "$?" -ne 0 ]]; then
			logmsg "$cmdpersist register error on $dev"
			err=1
		fi
	done
	...
	for dev in "${DEVICES[@]}"; do
		# Reserve the device
		set_cmd "$dev"
		$cmdpersist $cmdopts --out --preempt-abort --param-sark="$REMKEY" --param-rk="$OURKEY" --prout-type="$SCSI_PRTYPE" "$dev" >/dev/null 2>&1
...
do_remove() {
	...
	# TODO: check OURKEY is registered
...
do_readreservation() {
	for dev in "${DEVICES[@]}"; do
		...
		get_current_reservation_holder "$dev"
...
usage() {
	...
	echo "	Report registered keys and reservation."
	echo ""
	echo "Options:"
	echo "    --ourkey KEY"
	echo "        KEY is the local key."
	echo "    --removekey REMKEY"
	echo "        REMKEY is a another host's key."
	echo "    --prtype PRTYPE"
...
OPTIONS=$("$GETOPT" -o h -l help,ourkey:,removekey:,prtype:,aptpl,device:,vg: -n "${SCRIPTNAME}" -- "$@")
eval set -- "$OPTIONS"
