...
get_key_list_scsi() {
	...
	KEYS=( $($cmd $cmdopts --in --read-keys "$dev" 2>/dev/null | grep "    0x" | sort | xargs ) )
...
usage() {
	...
	echo ""
	echo "${SCRIPTNAME} start --ourkey KEY --prtype PRTYPE DEST"
	echo "	Register key and reserve device(s)."
	echo ""
	echo "${SCRIPTNAME} start --ourkey KEY --prtype PRTYPE --removekey REMKEY DEST"
	...
	echo ""
	echo "${SCRIPTNAME} devtest [--prtype PRTYPE] DEST"
...
OPTIONS=$("$GETOPT" -o h -l help,ourkey:,removekey:,key:,prtype:,ptpl,debug,device:,vg: -n "${SCRIPTNAME}" -- "$@")
eval set -- "$OPTIONS"
