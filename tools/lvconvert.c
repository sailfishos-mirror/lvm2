...
{
	struct lvcreate_params lp_create = { 0 };
	...
	if (seg_is_raid(seg)) {
		...
		if (!meta_name) {
			lp_create.lv_name = lv->name;
			lp_create.pvh = use_pvh;
			lp_create.extents = lv->size / vg->extent_size;

			if (!lv_create_integrity_metadata(cmd, vg, &lp_create))
				return_0;
		}

		ret = lv_add_integrity(lv, "external",
				       lp_create.integrity_meta_lv, meta_name,
				       set, use_pvh);
	}
	if (!ret)
		return 0;
