				...
				uint64_t data_offset_bytes)
{
	...
	if (dm_snprintf(data_path, sizeof(data_path), "%s/%s", dmdir, data_name) < 0) {
		log_error("Failed to create path for %s.", display_lvname(cachevol_lv));
		return 0;
	}

	if (dm_snprintf(offset_str, sizeof(offset_str), "%llu", (unsigned long long)data_offset_bytes) < 0) {
		log_error("Failed to create offset str.");
		return 0;
	}
...
static int _lvconvert_cache_repair_cachevol(struct cmd_context *cmd,
				   ...
				   struct dm_list *pvh)
{
	char tmp_lv_path[PATH_MAX];
	char tmp_lv_name[PATH_MAX];
	const char *tmp_file = NULL;
	const char *tmp_path = NULL; /* either tmp_file or tmp_lv_path */
	...
	/*
	 * For writethrough, the "repair" is to simply remove the
	 * cachevol without writing back.
	 */
	if (cache_mode == CACHE_MODE_WRITETHROUGH) {
		if (!_lvconvert_split_and_keep_cachevol(cmd, cache_lv, cachevol_lv)) {
			log_error("Failed to detach writethrough cachevol.");
			return_0;
		}
	...
	/*
	 * Create a temp file or temp LV to hold a repaired copy of
	 * cache metadata which is produced by cache_repair and
	 * consumed by cache_writeback, after which it is deleted.
	 * User can specify a --file <path> as the temp location,
	 * otherwise lvconvert creates a temp LV.
	 */
	if (arg_is_set(cmd, file_long_ARG)) {
		tmp_file = arg_str_value(cmd, file_long_ARG, NULL);

		if (!tmp_file || (tmp_file[0] != '/')) {
			log_error("Full path required to temp file.");
			tmp_file = NULL;
			goto out;
		}

		/* cache_repair wants the destination file preallocated */
		/* FIXME: do write odirect because that's what cache tools use */

		log_print("Creating %u MB temp file for repair %s",
			  (uint32_t)(metadata_len_sectors * 512 / 1048576), tmp_file);

		if (!(fp = fopen(tmp_file, "w+"))) {
			log_error("Failed to open file for metadata %s", tmp_file);
			tmp_file = NULL;
			goto out;
		}
		while (prealloc_sectors_written < metadata_len_sectors) {
			if (!fwrite(zero_sector_buf, 512, 1, fp)) {
				fclose(fp);
				log_error("Failed to allocate space for metadata in %s", tmp_file);
				goto out;
			}
		...
		lp.lv_name = tmp_lv_name;
		lp.extents = metadata_len_sectors / vg->extent_size;
		lp.segtype = get_segtype_from_string(cmd, SEG_TYPE_NAME_STRIPED);
		dm_list_init(&lp.tags);

		if (dm_snprintf(tmp_lv_name, sizeof(tmp_lv_name), "%s_tmprepair", cache_lv->name) < 0) {
			log_error("Failed to create temp LV name.");
			goto out;
		}

		if (find_lv(vg, tmp_lv_name)) {
			log_error("Existing temp file for repair exists and must be removed.");
			goto out;
		}

		log_print("Creating %u MB temp LV for repair %s/%s",
			  (uint32_t)(metadata_len_sectors * 512 / 1048576), vg->name, tmp_lv_name);

		if (!(tmp_lv = lv_create_single(vg, &lp))) {
			log_error("Failed to create temp LV for repair.");
			goto out;
		}

		if (!(tmp_dm_name = dm_build_dm_name(cmd->mem, vg->name, tmp_lv->name, NULL))) {
			log_error("Failed to create dm name for %s.", display_lvname(tmp_lv));
			goto out;
		}

		if (dm_snprintf(tmp_lv_path, sizeof(tmp_lv_path), "%s/%s", dm_dir(), tmp_dm_name) < 0) {
			log_error("Failed to create path for %s.", display_lvname(tmp_lv));
			goto out;
		}
		
		tmp_path = tmp_lv_path;
	}

	/* Disable syncing the cache during split (hacky) */
	cache_seg->cache_mode = CACHE_MODE_WRITETHROUGH;
	...
	if (!_run_cache_repair_from_cachevol(cmd, cachevol_lv, tmp_path)) {
	...
	if (!_run_cache_writeback_from_cachevol(cmd, cache_lv, cachevol_lv, tmp_path, data_offset_bytes)) {
		log_error("cache_writeback failed");
		goto_out;
	}

	log_print("cachevol writeback complete, previous cachevol %s is now unused.", cachevol_lv->name);
	ret = 1;
out:
	...
	if (tmp_file) {
		...
		if (!deactivate_lv(cmd, tmp_lv))
			log_error("Failed to deactivate temp LV %s.", display_lvname(tmp_lv));
		if (!lv_remove_single(cmd, tmp_lv, DONT_PROMPT, 1))
			log_error("Failed to remove temp LV %s.", display_lvname(tmp_lv));
	}
