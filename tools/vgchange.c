...
do_start:
	...
	if ((vg->pr & (VG_PR_REQUIRE|VG_PR_AUTOSTART)) && !persist_is_started(cmd, vg)) {
			             ...
			             struct processing_handle *handle)
{
	...
	/*
	 * TODO: if a number of hosts have a shared VG started,
	 * and PR is not being used, then one host does PR start,
	 * the other hosts will lose access to the VG.  Perhaps
	 * setpersist y should do an lvmlockd busy_vg check to
	 * see if other hosts are started, and if so require a
	 * confirmation?  The recommendation is to lockstop all
	 * hosts before setpersist y, then PR start will be
	 * required before lockstart.
	 */
