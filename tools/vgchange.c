			             ...
			             struct processing_handle *handle)
{
	...
	if (!strncmp(op, "check", 5)) {
		...
		/*
		 * Prevent the case where a VG is is started on multiple hosts,
		 * the VG has not been set to require PR (!vg->pr), and a user
		 * simply runs vgchange --persist start, which would succeed,
		 * but would result in other hosts suddenly losing device access
		 * to an in-use VG>
		 */
		if (!vg->pr && vg_is_shared(vg) && !vg_is_reserved(cmd, vg, NULL, NULL, NULL) &&
			             ...
			             struct processing_handle *handle)
{
	 ...
	 */
	if (on && vg_is_shared(vg) && !vg_is_reserved(cmd, vg, NULL, NULL, NULL) &&
	    lockd_vg_is_started(cmd, vg, NULL) && lockd_vg_is_busy(cmd, vg)) {
