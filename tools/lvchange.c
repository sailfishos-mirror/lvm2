				...
				struct logical_volume *lv)
{
	...
	if (dmeventd_monitor_mode() != DMEVENTD_MONITOR_IGNORE) {
		...
		if (!monitor_dev_for_events(cmd, lv, 0, dmeventd_monitor_mode() ? 1 : 0)) {
...
/*
 * Hidden sub LVs where we still allow lvchange (properties or refresh).
 *
 * lv_is_thin_pool_data: e.g. needed when the data sublv
 * is a cache lv and we need to change cache properties.
 */
static int _lv_is_changeable_component(const struct logical_volume *lv)
{
...
{
	if (!lv_is_visible(lv)) {
		if (_lv_is_changeable_component(lv))
			return 1;
