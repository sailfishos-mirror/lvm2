			 ...
			 struct dm_list *lvs_changed)
{
	...
	if (!dm_list_empty(lvs_changed) &&
	    !_detach_pvmove_mirror(cmd, lv_mirr, 1)) {
		  ...
		  struct logical_volume *lv_mirr, struct dm_list *lvs_changed)
{
	...
	if (!dm_list_empty(lvs_changed) &&
	    !_detach_pvmove_mirror(cmd, lv_mirr, arg_is_set(cmd, abort_ARG))) {
		log_error("ABORTING: Removal of temporary pvmove mirror %s failed.",
			  display_lvname(lv_mirr));
		return 0;
	}
	...
	/*
	 * Re-acquire cluster locks on active LVs before releasing
	 * pvmove0's lock.  During pvmove, LVs have no own lock --
	 * pvmove0's lock guards them.  If lock fails, deactivate.
	 */
	if (vg_is_shared(vg)) {
		struct lv_list *lvl2;
		int already_done;
		...
		dm_list_iterate_items(lvl, lvs_changed) {
			...
			if (!lv_is_active(holder) || !lockd_lv_uses_lock(holder))
				continue;
			...
			if (!lockd_lv(cmd, (struct logical_volume *)holder, "ex", LDLV_PERSISTENT)) {
				log_warn("WARNING: Failed to re-acquire lock for %s, deactivating.",
					 display_lvname(holder));
				if (!deactivate_lv(cmd, holder))
					log_warn("WARNING: Failed to deactivate %s.", display_lvname(holder));
			}
		}
	}
