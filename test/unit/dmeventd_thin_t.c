...
#define process_event thin_test_process_event
#define register_device thin_test_register_device
#define unregister_device thin_test_unregister_device
#include "daemons/dmeventd/libdevmapper-event.h"

/* Pool dm_task transport mocks are shared with the VDO unit test. */
#define dmeventd_lvm2_init thin_test_lvm2_init
#define dmeventd_lvm2_exit thin_test_lvm2_exit
#define dmeventd_lvm2_lock thin_test_lvm2_lock
#define dmeventd_lvm2_unlock thin_test_lvm2_unlock
#define dmeventd_lvm2_command thin_test_lvm2_command
#define dmeventd_lvm2_run thin_test_lvm2_run
#include "daemons/dmeventd/plugins/lvm2/dmeventd_lvm.h"

static int _thin_pool_setenv(const char *name, const char *value, int overwrite)
{
	T_ASSERT(overwrite);
	if (!strcmp(name, "LVM_RUN_BY_DMEVENTD"))
		T_ASSERT(!strcmp(value, "1"));
	else if (!strcmp(name, "DMEVENTD_THIN_POOL_DATA"))
		T_ASSERT(!strcmp(value, "51"));
	else {
		T_ASSERT(!strcmp(name, "DMEVENTD_THIN_POOL_METADATA"));
		T_ASSERT(!strcmp(value, "10"));
	}
	return 0;
}
#define DMEVENTD_UNIT_POOL_SETENV(name, value, overwrite) \
	_thin_pool_setenv(name, value, overwrite)
...
#include "daemons/dmeventd/plugins/thin/dmeventd_thin.c"

#define DMEVENTD_UNIT_LVM2_PREFIX thin_test
#include "dmeventd_unit_lvm2_lock.h"
int thin_test_lvm2_command(struct dm_pool *mem, char *buffer, size_t size,
			  const char *cmd, const char *device)
{
	return dm_snprintf(buffer, size, "lvm test-policy") >= 0;
}
int thin_test_lvm2_run(const char *cmd)
{
...
static void *_init(void)
{
	...
	_wait_result = 0;
	_wait_status = 0;
	_current.type = _fresh.type = "thin-pool";
	_current.status = _fresh.status = "0 10/100 10/100 - rw discard_passdown queue_if_no_space -";
	T_ASSERT(thin_test_register_device("test-pool", "test-uuid", 0, 0, &state));
	return state;
}
...
static void _exit_fixture(void *fixture)
{
	...
	state->pid = -1;
	T_ASSERT(thin_test_unregister_device("test-pool", "test-uuid", 0, 0, &fixture));
}

static void _event(void *fixture, enum dm_event_mask mask, unsigned policies)
{
	thin_test_process_event((struct dm_task *) &_current, mask, &fixture);
	T_ASSERT_EQUAL(_policies, policies);
}

static void _usage(void *fixture, unsigned meta, unsigned meta_size,
		   unsigned data, unsigned data_size, unsigned policies)
{
	char status[160];

	T_ASSERT(dm_snprintf(status, sizeof(status),
		"0 %u/%u %u/%u - rw discard_passdown queue_if_no_space -",
		meta, meta_size, data, data_size) >= 0);
...
/*
 * Strict greater-than step boundaries (>50%, >55%, ...), per thin plugin.
 */
static void _thresholds(void *fixture, int metadata)
{
	unsigned i;

	for (i = 0; i < DMEVENTD_UNIT_POOL_STRICT_THRESHOLD_STEPS; ++i)
		_usage(fixture, metadata ? dmeventd_unit_pool_strict_threshold_used[i] : 10, 100,
		       metadata ? 10 : dmeventd_unit_pool_strict_threshold_used[i], 100,
		       dmeventd_unit_pool_strict_threshold_calls[i]);
}
/*
 * Run the step-threshold scenario on the data percent. */
static void _data_thresholds(void *fixture) { _thresholds(fixture, 0); }
/*
 * Run the step-threshold scenario on the metadata percent. */
static void _metadata_thresholds(void *fixture) { _thresholds(fixture, 1); }
...
{
	_usage(fixture, 10, 100, 100, 100, 1);
	_usage(fixture, 10, 100, 100, 100, 1);
	_usage(fixture, 10, 100, 100, 100, 1);
}
...
static void _full_failure(void *fixture)
{
	...
	_usage(fixture, 10, 100, 100, 100, 1);
	_usage(fixture, 10, 100, 100, 100, 1);
	_usage(fixture, 10, 100, 100, 100, 2);
	_usage(fixture, 10, 100, 100, 100, 2);
	_usage(fixture, 10, 100, 100, 100, 2);
	_usage(fixture, 10, 100, 100, 100, 3);
	_policy_result = 1;
	_usage(fixture, 10, 100, 100, 100, 3);
	_usage(fixture, 10, 100, 100, 100, 3);
	_usage(fixture, 10, 100, 100, 100, 3);
	_usage(fixture, 10, 100, 100, 100, 3);
	_usage(fixture, 10, 100, 100, 100, 4);
	_usage(fixture, 10, 100, 100, 100, 4);
}

/*
 * Data and metadata thresholds are evaluated independently: a single event
 * may cross both, and each later crossing schedules its own policy call. */
static void _both(void *fixture)
{
	_usage(fixture, 50, 100, 50, 100, 0);
	_usage(fixture, 56, 100, 51, 100, 1);
	_usage(fixture, 56, 100, 56, 100, 2);
}
...
{
	_usage(fixture, 10, 100, 81, 100, 1);
	_usage(fixture, 10, 100, 40, 100, 1);
	_usage(fixture, 10, 100, 51, 100, 2);
	_usage(fixture, 10, 100, 76, 100, 3);
	_usage(fixture, 10, 100, 61, 100, 3);
	_usage(fixture, 10, 100, 80, 100, 4);
}

/*
 * A resize re-arms each axis at CHECK_MINIMUM.  Repeating the same block
 * counts after a resize still advances percent_check on every event, so
 * policy does not fire again until usage strictly exceeds the new armed step.
 */
static void _resize(void *fixture)
{
	_usage(fixture, 81, 100, 81, 100, 1);
	_usage(fixture, 162, 200, 162, 200, 2);
	_usage(fixture, 162, 200, 162, 200, 2);
	_usage(fixture, 162, 400, 162, 400, 2);
	_usage(fixture, 204, 400, 204, 400, 3);
}
...
static void _backoff(void *fixture)
{
	...
	_policy_result = 0;
	_usage(fixture, 10, 100, 51, 100, calls);
	...
	_usage(fixture, 10, 100, 51, 100, calls);
	_policy_result = 0;
	_usage(fixture, 10, 100, 56, 100, ++calls);
	_usage(fixture, 10, 100, 56, 100, calls);
	_usage(fixture, 10, 100, 56, 100, ++calls);
}
...
{
	_policy_result = 0;
	_usage(fixture, 51, 100, 51, 100, 1);
	_usage(fixture, 51, 100, 51, 100, 1);
	_policy_result = 1;
	_usage(fixture, 51, 100, 102, 200, 2);
	_policy_result = 0;
	_usage(fixture, 56, 100, 102, 200, 3);
	_policy_result = 1;
	_usage(fixture, 112, 200, 102, 200, 4);
}

/*
 * Malformed status and a missing or non-thin target must not disable
 * monitoring; a following valid status still works. */
static void _invalid(void *fixture)
{
	const char *bad[] = { NULL, "", "garbage", "0 10/100", "0 10/100 broken" };
	unsigned i;

	_usage(fixture, 10, 100, 51, 100, 1);
	...
	_current.type = NULL;
	_event(fixture, DM_EVENT_TIMEOUT, 1);
	_current.type = "thin-pool";
	_usage(fixture, 10, 100, 51, 100, 1);
	_usage(fixture, 10, 100, 56, 100, 2);
}

/*
 * A device-error mask runs the policy immediately without re-reading status
 * and without leaving stale percents from the prior event. */
static void _device_error(void *fixture)
{
	...
	T_ASSERT_EQUAL(state->data_percent, 0);
	T_ASSERT_EQUAL(state->metadata_percent, 0);
}
...
static void _error_refresh(void *fixture)
{
	...
	_usage(fixture, 10, 100, 10, 100, 0);
	_policy_result = 0;
	_current.status = "stale status must not be parsed";
	_fresh.status = "0 40/100 81/100 - rw discard_passdown queue_if_no_space -";
	...
	T_ASSERT_EQUAL(state->data_percent, 81 * DM_PERCENT_1);
	T_ASSERT_EQUAL(state->metadata_percent, 40 * DM_PERCENT_1);
}
...
static void _child_failure(void *fixture, int status)
{
	...
	_wait_result = 123;
	_wait_status = status;
	_usage(fixture, 10, 100, 51, 100, 0);
	T_ASSERT_EQUAL(state->pid, -1);
	_usage(fixture, 10, 100, 51, 100, 1);
...
void dmeventd_thin_tests(struct dm_list *all_tests)
{
	...
	T_ASSERT(ts);
#define TEST(name, desc, fn) register_test(ts, "/dmeventd/thin/" name, desc, fn)
	TEST("data-thresholds", "data crossings and full pool", _data_thresholds);
	TEST("metadata-thresholds", "metadata crossings and full pool", _metadata_thresholds);
	TEST("full", "full pool is not probed again", _full);
	TEST("full-failure", "failed policy on full pool keeps retrying", _full_failure);
	TEST("both", "independent thresholds share one policy", _both);
