...
mkdtemp() {
	...
	[[ -d "$destdir" ]] || die "DIR ('$destdir') does not exist."
...
STACKTRACE() {
	...
	if [[ "${LVM_TEST_PARALLEL:-0}" -eq 0 && -z "$RUNNING_DMEVENTD" && \
		! -f LOCAL_DMEVENTD ]] && DPID=$(pgrep -n dmeventd 2>/dev/null); then
	...
	if [[ "${LVM_TEST_NODEBUG:-0}" -eq 0 && -f TESTNAME ]]; then
		...
		for i in debug.log* ; do
			[[ -f "$i" ]] || break  # nothing is found (expands to debug.log*)
			name=${i##debug.log_}
			name=${name%%_*}
			[[ "$name" = "DEBUG" ]] && { name="$name$idx" ; idx=$(( idx + 1 )) ; }
			echo "<======== Debug log $i ========>"
			sed -e "s,^,## $name: ," "$i"
			mv -f "$i" "debug_${i#debug.}"
		done
		if [[ -e strace.log ]]; then
...
init_udev_transaction() {
	if [[ "$DM_UDEV_SYNCHRONIZATION" = 1 ]]; then
		local cookie
		cookie=$(dmsetup udevcreatecookie)
		# Cookie is not generated if udev is not running!
		[[ -z "$cookie" ]] || export DM_UDEV_COOKIE=$cookie
	fi
}

finish_udev_transaction() {
	if [[ "$DM_UDEV_SYNCHRONIZATION" = 1 && -n "${DM_UDEV_COOKIE-}" ]]; then
...
if [[ -z "${abs_top_builddir+varset}" && -z "${installed_testsuite+varset}" ]]; then
