...
# Seed the low blocks of the pool with the header and free them, then reload
# the pool so the freed low blocks are handed out again (see below): the chunk
# the VDO data LV's first extent lands on is one of those stale blocks.  The
# chunk has to be larger than 64 KiB so that offset 65536 shares a chunk with
# offset 0.
#
...
# Tile the pattern over the seed LV (100 MiB / 1 MiB chunks) so the low pool
# blocks carry the signature, then drop it: the blocks are freed but not
# zeroed (-Z n, no discards) and become the pool's stale free list.
seed_dev="$DM_DEV_DIR/$vg/seed"
lvcreate -V100M -T $vg/pool -n seed
cat $(printf 'pat %.0s' {1..100}) > "$seed_dev"
...
# Reload the pool to reset the kernel's data-block allocation cursor to block 0.
# The thin-pool data space map (dm-space-map-disk) resumes from a moving cursor
# past the last allocated block and only wraps once the pool is exhausted, so
# after seeding 100 blocks the freed low blocks are not handed out again until
# the cursor is reset.  Only the first ~16 pool blocks are consumed before the
# VDO data LV's first chunk, so the 100 MiB seed is a generous margin.
lvchange -an $vg/pool
lvchange -ay $vg/pool
