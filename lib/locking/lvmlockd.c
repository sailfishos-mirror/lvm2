			      ...
			      int creating_thin_pool, int creating_thin_volume)
{
	 ...
	 * enable lockd_init_lv() and lockd_lv().
	 */
...
int lockd_init_lv(struct cmd_context *cmd, struct volume_group *vg, struct logical_volume *lv,
		  struct lvcreate_params *lp, uint32_t flags)
{
	int lock_type_num = get_lock_type_from_string(vg->lock_type);
	...
	if (!lp->needs_lockd_init) {
		...
		if (flags & LDLV_CREATING_THIN_VOLUME) {
			/* can this happen? */
			log_error("lockd_init_lv thin invalid with LDLV_CREATING_THIN_VOLUME");
			return 0;
		}
		...
		if (!(flags & LDLV_CREATING_THIN_POOL)) {
			/* can this happen? */
			log_error("lockd_init_lv thin invalid without LDLV_CREATING_THIN_POOL");
			return 0;
		}

		/* create a new lock for a new thin pool */
		log_debug("lockd_init_lv creating new lock for thin pool");
		/* lock_args = "pending" below */
