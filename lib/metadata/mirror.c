...
{
	const struct logical_volume *lv, *prev_lv;
	...
	dm_list_iterate_items(lvl, lvs) {
		...
		dm_list_iterate_items(lvl2, lvs) {
			if (lvl2 == lvl)
				break;
			if ((prev_lv = find_active_pvmoved_lv(lvl2->lv)) == lv) {
				already_done = 1;
				break;
			}
		}
		 ...
		 * are already active (activate.c:2681) without reloading
		 * sub-device tables.  After pvmove layer removal, component
		 * LVs (e.g. _tmeta) still have kernel tables pointing to
		 * the old pvmove device; suspend+resume forces them to pick
		 * up the new post-pvmove mappings from committed metadata.
		 *
		 * This is safe because pvmove skips LVs whose holders are
		 * active on other nodes (pvmove.c:450), so all participating
		 * active LVs are guaranteed to have local DM devices.
		 */
