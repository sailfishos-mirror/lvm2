		      ...
		      struct lvresize_params *lp)
{
	 ...
	 * crypt_resize_command() needs to be decreased by the size of the luks
	 * header (LUKS1 usually 2MB, LUKS2 usually 16MB.)
	 */
	if (fsinfo.needs_crypt) {
		newsize_bytes_fs -= fsinfo.luks_header_bytes;
		log_print("File system size %llub is adjusted for LUKS header size %ub.",
			  (unsigned long long)newsize_bytes_fs, fsinfo.luks_header_bytes);
	}
	      ...
	      struct lvresize_params *lp)
{
 ...
 end_main:
	...
	if (lv_is_thin_pool(lv_top)) {
		if (!update_pool_lv(lv_top, 1))
			goto_out;
		if (is_extend)
			pool_check_overprovisioning(lv_top);
	}
