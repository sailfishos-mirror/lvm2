				     ...
				     struct integrity_settings *settings, int lbs_4k, int lbs_512)
{
	...
	if (!strcmp(cmd->name, "lvcreate")) {
		...
		if (!rv || !fs_block_size) {
			if (lbs_4k) {
				/* require the user to specify --raidintegrityblocksize 4096 as confirmation */
				if (settings->block_size == 4096) {
					settings->block_size = 4096;
				} else {
					log_error("Set --raidintegrityblocksize 4096 if the file system block size is not less than 4096.");
					goto_bad;
				}
			} else {
				/* --raidintegrityblocksize must be 512 or unspecified */
				if (settings->block_size && (settings->block_size != 512)) {
					log_error("Set --raidintegrityblocksize 512 for unknown file system block size and logical block size 512.");
					goto_bad;
				}
				settings->block_size = 512;
			}
			log_print("Using integrity block size %u for unknown file system block size and logical block size %u.",
				  settings->block_size, lbs_4k ? 4096 : 512);
			goto out;
		}
