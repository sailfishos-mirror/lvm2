...
static int _set_integrity_block_size(struct cmd_context *cmd, struct logical_volume *lv,
				     struct integrity_settings *settings, int lbs_4k, int lbs_512)
{
	char pathname[PATH_MAX];
	struct device *fs_dev;
	uint32_t integrity_block_size = 0;
	...
	if (!strcmp(cmd->name, "lvcreate")) {
		if (lbs_4k && lbs_512) {
			...
			log_warn("WARNING: unknown logical block size for LV devices, assuming 512.");
			integrity_block_size = 512;
		} else if (lbs_4k) {
			integrity_block_size = 4096;
		} else if (lbs_512) {
			integrity_block_size = 512;
		} else {
			goto_bad;
		...
		if (!settings->block_size)
			settings->block_size = integrity_block_size;
		else if ((settings->block_size != integrity_block_size) && (lbs_4k || lbs_512)) {
			log_print("Using integrity block size %u on devices with logical block size %u",
				  settings->block_size, integrity_block_size);
		}
		...
		if (!get_fs_block_size(fs_dev, &fs_block_size)) {
			integrity_block_size = 512;
			log_print("Using integrity block size %u for unknown file system block size.", integrity_block_size);
		} else {
			if ((fs_block_size == 512) || (fs_block_size == 4096)) {
				integrity_block_size = fs_block_size;
				log_print("Using integrity block size %u to match file system.", integrity_block_size);
			} else if (fs_block_size && (fs_block_size == settings->block_size)) {
				integrity_block_size = fs_block_size;
				log_print("Using integrity block size %u for file system block size %u.", integrity_block_size, fs_block_size);
		...
		if (!settings->block_size)
			settings->block_size = integrity_block_size;
		else if (settings->block_size == integrity_block_size)
			settings->block_size = integrity_block_size;
		else if (settings->block_size == 512) {
			if (fs_block_size && (fs_block_size != 512))
				log_print("Using integrity block size 512 with detected file system block size %u", fs_block_size);
		} else if (fs_block_size && (settings->block_size != fs_block_size)) {
			log_error("Cannot use integrity block size %u with unmatching file system block size %u",
				  settings->block_size, fs_block_size);
			goto_bad;
		} else if (settings->block_size != integrity_block_size) {
			log_error("Cannot use integrity block size %u with unknown file system block size which defaults to %u.",
				  settings->block_size, integrity_block_size);
			goto_bad;
		}
	}
			     ...
			     struct dm_list *pvh, struct logical_volume *lv_imeta_0)
{
	 ...
	 * integrity block size chosen based on device logical block size for lvcreate,
	 * or file system block size for lvconvert.
	 */
	if (!_set_integrity_block_size(cmd, lv, settings, lbs_4k, lbs_512))
		goto_bad;
