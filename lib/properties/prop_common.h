 ...
 */
#ifndef LVM_PROP_COMMON_H
#define LVM_PROP_COMMON_H
...
#define FIELD(type, strct, field_type, head, field, width, fn, id, desc, settable) \
	{ type, #id, settable, (field_type == STR || field_type == STR_LIST || field_type == TIM),\
	((field_type == NUM) || (field_type == BIN) || (field_type == SIZ) || (field_type == PCT) || (field_type == SNUM)),\
	((field_type == SNUM) || (field_type == PCT)), { .integer = 0 }, _ ## id ## _get, _ ## id ## _set },
