		    ...
		    struct lvmcache_vgsummary *vgsummary, uint64_t *mda_free_sectors)
{
	 ...
	 * the checksum/size, and if so fill in the other vgsummary
	 * info from lvmcache.  If we do this, vgsummary->vgname
	 * will be set, and that controls whether we pass 1 or 0
	 * to text_read_metadata_summary() as the checksum_only
	 * flag which controls if the metadata is parsed.
	 */
