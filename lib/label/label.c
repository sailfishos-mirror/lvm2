				  ...
				  int *scan_failed_count)
{
 ...
 start_more:
	 ...
	 *
	 * FIXME: preallocate a fixed buffer that we can revert to using
	 * for remaining scans.
	 */
 ...
 check_aio:
	 ...
	 *
	 * FIXME: As a further optimization, we could submit more reads
...
static int _label_scan_async(struct cmd_context *cmd, int skip_cached, int *nomem)
{
	...
	/* FIXME: if this fails, goto bad */
	_label_scan_list_async(cmd, &_label_read_list, async_event_count, buf_len, &scan_failed_count);
...
static int _label_scan_devs_async(struct cmd_context *cmd, struct dm_list *devs, int *nomem)
{
	...
	/* FIXME: if this fails, goto bad */
	_label_scan_list_async(cmd, &tmp_label_read_list, async_event_count, buf_len, &scan_failed_count);
