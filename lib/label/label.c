...
static int _label_scan_async(struct cmd_context *cmd, int skip_cached, int *nomem)
{
	...
	log_debug_devs("Scanning all devices using async io with %d async_events and scan_size %d bytes.",
			async_event_count, buf_len);
 ...
 bad:
	 ...
	 * Tell caller to try _lite variation which uses less mem.
	 */
...
static int _label_scan_sync(struct cmd_context *cmd, int skip_cached, int *nomem)
{
	...
	log_debug_devs("Scanning all devices using synchronous io with scan_size %d bytes.", buf_len);
...
static int _label_scan_lite(struct cmd_context *cmd, int skip_cached)
{
	struct dev_iter *iter;
	struct device *dev;
	struct lvmcache_info *info;
	...
	log_debug_devs("Scanning all devices using synchronous io with no scan_cache and scan_size %d bytes.", buf_len);
	...
	while ((dev = dev_iter_get(iter))) {
		...
		log_debug_devs("Reading sectors from device %s sync lite", dev_name(dev));
...
static int _label_scan_devs_lite(struct cmd_context *cmd, struct dm_list *devs)
{
	struct label_read_data ld;
	struct device_list *devl;
	struct device *dev;
	...
	log_debug_devs("Scanning devs sync lite.");
...
int label_scan_force(struct cmd_context *cmd)
{
	...
	if (cmd->use_aio)
		if (_label_scan_async(cmd, 0, &nomem))
			return 1;

	if (!nomem && cmd->use_scan_cache)
		if (_label_scan_sync(cmd, 0, &nomem))
			return 1;

	if (nomem || (!cmd->use_aio && !cmd->use_scan_cache))
		if (_label_scan_lite(cmd, 0))
			return 1;
...
/*
 * scan_async && scan_cache
 * . uses label_scan_async
 * . falls back to label_scan_sync if aio setup fails,
 * . falls back to label_scan_lite if nomem failure from either
 *
 * !scan_async && scan_cache
 * . uses label_scan_sync
 * . falls back to label_scan_lite if nomem failure
 *
 * !scan_async && !scan_cache
 * . uses label_scan_lite
 *
 * scan_async && !scan_cache:
 * . invalid config, use default settings
 */
...
int label_scan(struct cmd_context *cmd)
{
	...
	if (cmd->use_aio)
		if (_label_scan_async(cmd, 1, &nomem))
			return 1;

	if (!nomem && cmd->use_scan_cache)
		if (_label_scan_sync(cmd, 1, &nomem))
			return 1;

	if (nomem || (!cmd->use_aio && !cmd->use_scan_cache))
		if (_label_scan_lite(cmd, 1))
			return 1;
...
int label_scan_devs(struct cmd_context *cmd, struct dm_list *devs)
{
	...
	if (cmd->use_aio && _ac)
		if (_label_scan_devs_async(cmd, devs, &nomem))
			return 1;

	if (!nomem && cmd->use_scan_cache)
		if (_label_scan_devs_sync(cmd, devs, &nomem))
			return 1;

	if (nomem || (!cmd->use_aio && !cmd->use_scan_cache))
		if (_label_scan_devs_lite(cmd, devs))
			return 1;
