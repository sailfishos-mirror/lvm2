...
int setup_devices_file(struct cmd_context *cmd)
{
	...
	if (cmd->run_by_dmeventd) {
		...
		/* TODO: check for and use dmeventd.devices if it exists */
...
int setup_devices(struct cmd_context *cmd)
{
	 ...
	 *
	 * TODO: a possible exception here is if we can see that pvcreate is
	 * creating the first PV on the system and can conclude the system is
	 * being installed.  In that case we'd like the the system to be
	 * installed with the system devices file in place.  Perhaps we
	 * could create the system.devices_new file but avoid renaming it
	 * to system.devices immediately. If label_scan does not see any
	 * other PVs on the system, do the rename to system.devices, and
	 * if label_scsan does find other PVs, then unlink system.devices_new.
	 */
	if (!file_exists && !cmd->devicesfile &&
	    (!strncmp(cmd->name, "pvcreate", 8) || !strncmp(cmd->name, "vgcreate", 8))) {
		log_debug("Devices file for system not created.");
		cmd->enable_devices_file = 0;
		goto scan;
	}
	...
	if (!file_exists) {
		...
		log_print("Created new devices file %s", cmd->devices_file_path);
	} else {
		/*
