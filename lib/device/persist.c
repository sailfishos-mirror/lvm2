...
static int dev_find_key_scsi(struct cmd_context *cmd, struct device *dev,
			...
			int find_all, int *found_count, uint64_t **found_all)
{
	...
	if (dev_is_nvme(dev))
		return dev_find_key_nvme(cmd, dev, find_key, found_key,
					 find_host_id, found_host_id_key,
					 find_all, found_count, found_all);
	else
		return dev_find_key_scsi(cmd, dev, find_key, found_key,
					 find_host_id, found_host_id_key,
					 find_all, found_count, found_all);
}
...
static int vg_is_registered_by_key(struct cmd_context *cmd, struct volume_group *vg, uint64_t key, int *partial)
{
	...
	dm_list_iterate_items(pvl, &vg->pvs) {
		...
		if (!dev_find_key(cmd, dev, key, &found, 0, NULL, 0, NULL, NULL)) {
...
static int vg_is_registered_by_host_id(struct cmd_context *cmd, struct volume_group *vg, int host_id, uint64_t *key, uint32_t *gen, int *partial)
{
	...
	dm_list_iterate_items(pvl, &vg->pvs) {
		...
		if (!dev_find_key(cmd, dev, 0, NULL, host_id, &found_key, 0, NULL, NULL)) {
			log_error("Failed to read persistent reservation key on %s", dev_name(dev));
			errors++;
			continue;
		}
...
int persist_key_update(struct cmd_context *cmd, struct volume_group *vg, uint32_t prev_gen)
{
	...
	if (!read_key_file(cmd, vg, our_key_buf, &our_key_val, &file_host_id, &key_gen) ||
	    (file_host_id != local_host_id)) {

		if (our_key_val && file_host_id && (file_host_id != local_host_id)) {
			/* would happen if the host_id was changed */
			log_debug("persist_key_update: ignore key file with incorrect host_id %d", file_host_id);
			persist_key_file_remove(cmd, vg);
		} else {
			      ...
			      char *local_key, int local_host_id)
{
	...
	dm_list_iterate_items(pvl, &vg->pvs) {
		...
		if (!dev_find_key(cmd, dev, 0, NULL, 0, NULL, 1, &count, &keys)) {
			error_count++;
			continue;
		}
			  	  ...
			  	  uint64_t our_key_val, struct device *check_dev)
{
	...
	if (!dev_find_key(cmd, check_dev, 0, NULL, 0, NULL, 1, &old_count, &old_vals)) {
	...
	dm_list_iterate_items(pvl, &vg->pv_write_list) {
		...
		if (!dev_find_key(cmd, dev, 0, NULL, 0, NULL, 1, &new_count, &new_vals)) {
...
int persist_start_extend(struct cmd_context *cmd, struct volume_group *vg)
{
	...
	/*
	 * Try to narrow the cases in which we need to attempt reading
	 * our PR key from a device to check if PR is in use.
	 */
	 ...
	 * not in use.
	 */
	if (!dev_find_key(cmd, check_dev, our_key_val, &found, 0, NULL, 0, NULL, NULL))
		return 1;
	if (!found)
		return 1;
 ...
 do_extend:
	...
	dm_list_iterate_items(pvl, &vg->pv_write_list) {
		...
		if (!dev_find_key(cmd, dev, our_key_val, &found, 0, NULL, 0, NULL, NULL)) {
			log_error("Failed to read persistent reservation key on %s", dev_name(dev));
			errors++;
			continue;
		}
