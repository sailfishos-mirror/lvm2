...
int persist_is_started(struct cmd_context *cmd, struct volume_group *vg, int may_fail)
{
	...
	dm_list_iterate_items(pvl, &vg->pvs) {
		...
		if (!dev_read_reservation(cmd, dev, &holder, &prtype)) {
			log_debug("failed PR read on %s", dev_name(dev));
			goto out;
		}

		if (!prtype) {
			log_debug("no PR on %s", dev_name(dev));
			goto out;
		}

		if ((prtype != PR_TYPE_WE) && (prtype != PR_TYPE_WEAR)) {
			log_debug("wrong PR type %s on %s", prtype_to_str(prtype), dev_name(dev));
			goto out;
		}

		if ((prtype == PR_TYPE_WE) && (holder != our_key_val)) {
			log_debug("other PR holder 0x%llx on %s",
				...
				char *local_key, int local_host_id)
{
	...
	if (pv_error_reg || pv_error_res || pv_no_reg || pv_no_res || pv_res_we_other || pv_res_wear_other || pv_res_other_type) {
