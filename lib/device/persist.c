...
int persist_stop(struct cmd_context *cmd, struct volume_group *vg)
{
	...
	if (lockd_vg_is_started(cmd, vg, &cur_gen)) {
		log_error("VG locking should be stopped before PR stop (vgchange --lockstop)");
		return 0;
	}
			  	  ...
			  	  struct device *check_dev)
{
	...
	dm_list_iterate_items(pvl, &vg->pv_write_list) {
		...
		if (!dev_find_key(cmd, check_dev, 0, NULL, 0, NULL, 1, &new_count, &new_vals)) {
		...
		if (new_count != old_count) {
			log_error("PR key count %d (expect %d) found on %s",
...
int persist_start_extend(struct cmd_context *cmd, struct volume_group *vg)
{
 ...
 do_extend:
	...
	if (vg_is_shared(vg))
		return _persist_extend_shared(cmd, vg, check_dev);
