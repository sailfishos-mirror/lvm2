...
void device_ids_validate(struct cmd_context *cmd)
{
	...
	if (update_file) {
		...
		/* Use non-blocking lock since it's not essential to
		...
		if (!lock_devices_file_try(cmd, LOCK_EX, &held)) {
			log_print("Skip devices file update (busy).");
		} else {
			if (device_ids_version_unchanged(cmd))
				device_ids_write(cmd);
			else
				log_print("Skip devices file update (changed).");
		}
		if (!held)
...
{
	if (!cmd->devices_file || !strlen(cmd->devices_file) || !cmd->devices_file_path[0])
		return 0;
	return 1;
}
...
static int _lock_devices_file(struct cmd_context *cmd, int mode, int nonblock, int *held)
{
	...
	if (!(lock_dir = find_config_tree_str(cmd, global_locking_dir_CFG, NULL)))
		return 0;
	if (dm_snprintf(_devices_lockfile, sizeof(_devices_lockfile), "%s/D_%s", lock_dir, cmd->devices_file) < 0) {
		return 0;
	}
