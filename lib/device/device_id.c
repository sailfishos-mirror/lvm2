...
static int _read_sys_block(struct cmd_context *cmd, struct device *dev, const char *suffix, const char **idname)
{
 ...
 retry:
	...
	if (buf[0]) {
		if (prim)
			log_print("Using primary device_id for partition %s.", dev_name(dev));
...
static struct use_id *_get_uid_for_pvid(struct cmd_context *cmd, char *pvid)
{
	struct use_id *uid;
...
static struct use_id *_get_uid_for_devname(struct cmd_context *cmd, char *devname)
{
	struct use_id *uid;
		  ...
		  const char *idtype_arg, const char *id_arg)
{
	...
	if (dev->id || (dev->flags & DEV_MATCHED_USE_ID)) {
		log_print("Resetting device_id for PVID %s", pvid);
		dev->id = NULL;
		dev->flags &= ~DEV_MATCHED_USE_ID;
	}

	uid1 = _get_uid_for_pvid(cmd, pvid);
	uid2 = _get_uid_for_devname(cmd, dev_name(dev));

	if (!uid1 && !uid2) {
		if (!(uid = zalloc(sizeof(struct use_id))))
			return_0;
	} else if (uid1 && uid2 && (uid1 == uid2)) {
		uid = uid1;
		dm_list_del(&uid->list);
		log_debug("Update uid matching both %s %s", uid->devname, uid->pvid);
	} else if (uid1 && !uid2) {
		uid = uid1;
		dm_list_del(&uid->list);
		log_print("Update uid matching pvid %s not devname %s vs %s", uid->pvid, dev_name(dev), uid1->devname);
	} else if (!uid1 && uid2) {
		if (!(uid = zalloc(sizeof(struct use_id))))
			return_0;
		free(uid2->devname);
		uid2->devname = NULL;
		log_print("Clearing uid devname %s for PVID %s in devices_file", uid2->devname, uid2->pvid);
	} else if (uid1 && uid2 && (uid1 != uid2)) {
		uid = uid1;
		/* we know the uid2 entry has the wrong devname so clear it */
		free(uid2->devname);
		uid2->devname = NULL;
		dm_list_del(&uid->list);
		log_print("Clearing uid devname %s for PVID %s in devices_file", uid2->devname, uid2->pvid);
...
id_done:
	...
	/* FIXME: if we're updating an existing entry,
	   leave in place the correct fields,
	   free other fields we're replacing */
	...
	log_print("Add %s %s %s", dev_name(dev), uid->idname, uid->pvid);
