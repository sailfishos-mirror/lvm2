...
int device_ids_read(struct cmd_context *cmd)
{
	...
	/*
	 * Should only be one case where device_ids_read()
	 * is called a second time: pvscan --cache -aay
	 * where the initial pvscan does it, and then
	 * activation uses process_each_vg which does it.
	 *
	 * If we wanted to redo it, we'd need to
	 * free_uids(&cmd->use_device_ids) and
		  ...
		  const char *idtype_arg, const char *id_arg)
{
	 ...
	 * "md_uuid_udev" from udev MD_UUID=ddea39b4:664929c0:8058b96d:164de08d
	 *
...
void device_ids_match(struct cmd_context *cmd)
{
	...
	dm_list_iterate_items(uid, &cmd->use_device_ids) {
		...
		log_print("No system device matched to devices file entry with devname %s idtype %s idname %s pvid %s.",
...
void device_ids_validate(struct cmd_context *cmd, int force_update)
{
	...
	dm_list_iterate_items(uid, &cmd->use_device_ids) {
		...
		if (!cmd->filter->passes_filter(cmd, cmd->filter, dev, "persistent")) {
			...
			log_print("Devices file entry for %s excluded by filter.", dev_name(dev));
			continue;
		}
		...
		if (dev->pvid[0]) {
			if (!uid->pvid || strcmp(dev->pvid, uid->pvid)) {
				if ((uid->idtype == DEV_ID_TYPE_DEVNAME) && uid->pvid && (uid->pvid[0] != '.')) {
					...
					log_print("Device %s undo devname id match with wrong device PVID.", dev_name(dev));
					dev->flags &= ~DEV_MATCHED_USE_ID;
					dev->id = NULL;
					uid->dev = NULL;
					continue;
			...
			if (uid->pvid && (uid->pvid[0] != '.')) {
				if (uid->idtype == DEV_ID_TYPE_DEVNAME) {
					/* undo the uid/dev match for this entry */
					log_print("Device %s undo devname id match with unknown device PVID.", dev_name(dev));
					dev->flags &= ~DEV_MATCHED_USE_ID;
					dev->id = NULL;
					uid->dev = NULL;
					continue;
