				  ...
				  int *search_count, int noupdate)
{
	...
	/*
	 * A previous command did this search for devnames and
	 * found nothing, so it created the searched tmp file to
	 * tell us not to bother.  Without this, a device that's
	 * detached from the system (and identified by devname)
	 * would cause every command to search for it.  If the
	 * detached device is later attached, it will generate
	 * a pvscan, and pvscan will unlink the searched file,
	 * so a subsequent lvm command will do the search again.
	 */

	/*
	if (_searched_file_exists(cmd)) {
		log_debug("Skip search for devnames.");
		return;
	}
	*/
	...
	/*
	 * Prevent more devname searches.
	 */
	/*
	if (not_found && !found)
		_touch_searched_file(cmd);
	*/
}
