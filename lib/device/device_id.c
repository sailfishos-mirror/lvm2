...
static int _dev_has_mpath_uuid(struct cmd_context *cmd, struct device *dev, const char **idname_out)
{
	...
	if (idname) {
		...
		/*
		 * If it's a kpartx partitioned mpath device the dm uuid will
		 * be part%d-mpath-<wwid>  e.g. part1-mpath-abc...
		 */
		if (!strncmp(idname, "part", 4)) {
			const char *dash = strchr(idname, '-');
			if (dash && (strlen(dash) > 6) && !strncmp(dash + 1, "mpath-", 6)) {
				*idname_out = idname;
				return 1;
			}
		}
...
static int _dev_has_stable_id(struct cmd_context *cmd, struct device *dev)
{
	...
	dm_list_iterate_items(did, &dev->ids) {
		if (did->idtype != DEV_ID_TYPE_DEVNAME)
			return 1;
	}

	if (_read_sys_block(cmd, dev, "device/wwid", NULL))
		return 1;

	if (_read_sys_block(cmd, dev, "wwid", NULL))
		return 1;

	if (_read_sys_block(cmd, dev, "device/serial", NULL))
		return 1;

	if ((MAJOR(dev->dev) == cmd->dev_types->device_mapper_major) &&
	    _read_sys_block(cmd, dev, "dm/uuid", NULL))
		return 1;

	if ((MAJOR(dev->dev) == cmd->dev_types->md_major) &&
	    _read_sys_block(cmd, dev, "md/uuid", NULL))
		return 1;

	if ((MAJOR(dev->dev) == cmd->dev_types->loop_major) &&
	    _read_sys_block(cmd, dev, "loop/backing_file", NULL))
		return 1;
		  ...
		  const char *idtype_arg, const char *id_arg)
{
	 ...
	 * (kpartx on mpath handled by dev_has_mpath_uuid)
	 */
...
static int _match_uid_deviceid_to_dev(struct cmd_context *cmd, struct use_id *uid, struct device *dev)
{
	...
	if (part != uid->part) {
		...
		log_debug("compare mis %s %s part %d to %s part %d",
			  idtype_to_str(uid->idtype), uid->idname ?: ".", uid->part, dev_name(dev), part);
		*/
		return 0;
	}
		...
		if (did->idtype == uid->idtype) {
			if (did->idname && !strcmp(did->idname, uid->idname)) {
				...
				log_debug("compare match %s %s to %s",
					  idtype_to_str(uid->idtype), uid->idname, dev_name(dev));
				return 1;
			} else {
				/*
				log_debug("compare mis %s %s to %s %s",
			  		  idtype_to_str(uid->idtype), uid->idname ?: ".", dev_name(dev),
					  ((did->idtype != DEV_ID_TYPE_DEVNAME) && did->idname) ? did->idname : "");
				*/
				return 0;
			}
		}
...
#if 0
		/*
		 * TODO: search_for_devnames = none|auto|all
		 * for "all", don't use dev_has_stable_id().
		 * For testing on dm devs we need to use all.
		 */
