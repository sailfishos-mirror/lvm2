...
int device_ids_read(struct cmd_context *cmd)
{
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		if (buf[0]) {
			if (buf[0] && (buf[0] != '.'))
				du->idname = strdup(buf);
		}

		if (devname) {
			_copy_idline_str(devname, buf, PATH_MAX);
			if (buf[0] && (buf[0] != '.'))
				du->devname = strdup(buf);
		}
		  ...
		  const char *idtype_arg, const char *id_arg)
{
	 ...
	 * of device, e.g. loop, mpath, crypt, lvmlv, md, nbd, etc
	 *
	...
	/*
	 * TODO: dev_has_kpartx_dm_uuid(dev) for loop partitions.
	 */
	...
	if (du_dev) {
		...
		if (du_devid && (du_devid != du_dev)) {
			log_warn("WARNING: device %s (%s) and %s (%s) have duplicate device ID.",
				 ...
				 du_pvid->dev ? dev_name(du_pvid->dev) : ".", du_pvid->idname);
		}
		...
		if (du_devname && (du_devname != du_dev)) {
			...
			log_print("Clearing stale devname %s for PVID %s",
				  du_devname->devname, du_devname->pvid);
		...
		if (check_idname && !strcmp(check_idname, id->idname)) {
			...
			if (!label_scan_open(du_devid->dev))
				log_print("Cannot open %s", dev_name(du_devid->dev));
			...
			if (dev_is_partitioned(cmd->dev_types, du_devid->dev)) {
				...
				if ((ret1 == 2) && (devt1 == du_devid->dev->dev))
					log_print("WARNING: remove partitioned device %s from devices file.", dev_name(du_devid->dev));
			} else {
...
void device_id_pvremove(struct cmd_context *cmd, struct device *dev)
{
	...
	if (!(du = get_du_for_dev(cmd, dev))) {
		log_warn("WARNING: use_devices does not include %s", dev_name(dev));
		return;
	}
...
static int _match_du_deviceid_to_dev(struct cmd_context *cmd, struct dev_use *du, struct device *dev)
{
	struct dev_id *id;
	const char *idname;
	int part;
...
int device_ids_match_dev(struct cmd_context *cmd, struct device *dev)
{
	...
	if ((du = _get_du_for_devname(cmd, dev_name(dev)))) {
		if (_match_du_deviceid_to_dev(cmd, du, dev))
			return 1;
	}
...
void device_ids_match(struct cmd_context *cmd)
{
	...
	if (cmd->enable_devices_list) {
		dm_list_iterate_items(du, &cmd->use_devices) {
			...
			if (!(du->dev = dev_cache_get(cmd, du->devname, NULL))) {
				log_print("Device not found for %s.", du->devname);
			} else {
	...
	dm_list_iterate_items(du, &cmd->use_devices) {
		...
		log_print("No device matches %s %s %s %s %d",
			  idtype_to_str(du->idtype),
			  du->idname ?: ".",
			  du->devname ?: ".",
			  du->pvid ?: ".",
...
void device_ids_validate(struct cmd_context *cmd, int *device_ids_invalid, int noupdate)
{
	...
	dm_list_iterate_items(du, &cmd->use_devices) {
		...
		if (dev->pvid[0]) {
			if (!du->pvid || memcmp(dev->pvid, du->pvid, ID_LEN)) {
				log_print("Device %s has PVID %s (devices file %s)",
					  dev_name(dev), dev->pvid, du->pvid ?: ".");
				if (du->pvid)
					free(du->pvid);
				if (!(du->pvid = strdup(dev->pvid)))
					stack;
				update_file = 1;
		...
		if (!du->devname || strcmp(dev_name(du->dev), du->devname)) {
			log_print("Device %s has updated name (devices file %s)",
				   dev_name(du->dev), du->devname ?: ".");
			if (du->devname)
				free(du->devname);
			if (!(du->devname = strdup(dev_name(du->dev))))
				stack;
			update_file = 1;
			*device_ids_invalid = 1;
		}
	...
	dm_list_iterate_items(du, &cmd->use_devices) {
		...
		log_print("Devices file PVID %s matched to wrong device %s with PVID %s",
			  du->pvid, dev_name(dev), dev->pvid[0] ? dev->pvid : ".");
...
/*
 * Read pv_header for each du to get pvid.
 * Compare with du->pvid, and fix du->pvid if different.
 */
				  ...
				  int *search_count, int noupdate)
{
	...
	dm_list_iterate_items(du, &cmd->use_devices) {
		...
		log_print("No device found for devices file PVID %s.", du->pvid);
		if (search_count)
			(*search_count)++;
	}
	...
	dm_list_iterate_items(devl, &search_devs) {
		...
		/*
		 * This is a somewhat subtle optimization.  We only need to
		 * check devs that would use DEV_ID_TYPE_DEVNAME themselves as
		 * alternatives to the missing DEV_ID_TYPE_DEVNAME entry.
		 * i.e. a ID_TYPE_DEVNAME entry would not appear on a device
		 * that has a wwid and would use ID_TYPE_SYS_WWID.  So, if a
		 * dev in the search_devs list has a proper/stable device id
		 * (e.g. wwid, serial, loop, mpath), then we don't need to read
		 * it to check for missing PVIDs.
		 * 
		 * search_for_devnames="all" means we should search every device,
	...
	/* FIXME: devices with idtype devname that are simply detached from
	   ...
	   After a command searches for missing devices with IDNAME=devname,
	   it could touch /run/lvm/<devicefile_name>_searched.  Before the next command
	   searches for missing devices it would check for <df>_searched 
	   and skip searching if the file exists.  When pvscan --cache is run
	   for a new dev, it would remove <df>_searched so the next lvm
	   command would search again, since the missing dev may have appeared
	   (and triggered the pvscan).  We don't want pvscan itself managing
	   the <df>_searched file.  (The list of missing pvids could be
	   written in the <df>_searched file and pvscan only remove it if
	   the new pvid is in the file, but this is more likely to require
	   carefully locking of the file that we'd want to avoid.) */
