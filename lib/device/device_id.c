...
void device_ids_find_renamed_devs(struct cmd_context *cmd, struct dm_list *dev_list, int noupdate)
{
	...
	/* FIXME: devices with idtype devname that are simply detached from
	   ...
	   it could touch /run/lvm/searched_devnames.  Before the next command
	   searches for missing devices it would check for searched_devnames
	   and skip searching if the file exists.  When pvscan --cache is run
	   for a new dev, it would remove searched_devnames so the next lvm
	   ...
	   the searched_devnames file.  The list of missing pvids could be
	   written in the searched_devnames file. */
