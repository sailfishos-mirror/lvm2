...
id_done:
	...
	if (uid_pvid) {
		...
		/* FIXME: what about two devs with two pvids having the same devid?
		   we want two entries, one for each pvid, each having the same devid.
		   we need to verify both devs have same devid, and both devs have
		   different pvids by reading them.  if one of the entries is not
		   found anywhere, then remove it? */
