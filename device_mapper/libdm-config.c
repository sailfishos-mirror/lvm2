	...
	if (p->t == TOK_SECTION_B) {
		...
		if (p->stop_after_section && (--p->section_indent == 1)) {
			if (!strcmp(str, p->stop_after_section)) {
				/* config parsing is finished after section
				 * so try to find the 'last' } in parsed buffer to continue
				 * filling remaining nodes */

				/* looking backward for the sequence "\n}\n" */
				for (p->te = p->fe - 1; p->te > p->tb; --p->te)
					if ((p->te[-2] == '\n') &&
					    (p->te[-1] == '}') &&
					    (p->te[ 0] == '\n')) {
						p->t = TOK_SECTION_E;
						//p->stop_after_section = NULL;
						break;
					}
			}
		}
